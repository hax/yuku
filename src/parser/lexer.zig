...
/// `chars`, searched 16 bytes at a time; `src.len` on a miss. Source
/// length fits u32 (asserted at `Lexer.init`).
...
pub const Lexer = struct {
    ...
    fn scanBlockComment(self: *Lexer) LexicalError!void {
        ...
        // multi-line body: vectorized search for the two-byte '*/' sequence
        // (star and slash masks combined per lane). line leads are " * " -
        // star without a following slash - so they never restart the scan,
        // and the combined mask only fires on real terminators. windows
        // overlap by one byte so a '*/' straddling two windows is still
        // found; the overlap can never reach the opener's star, since the
        // walk began at start + 2.
        var w = pos - 1;
        while (w + 16 <= src.len) {
