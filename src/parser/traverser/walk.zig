...
fn walkChildren(
    ...
    switch (data) {
        // heritage-split payload: walk the reassembled logical Class so
        // that extends and implements children are visited too
        .class => |core| return walkStructFields(C, V, visitor, ast.Class, ctx.tree.classOf(core), ctx),
        inline else => |node| {
            const T = @TypeOf(node);
